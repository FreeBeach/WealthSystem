...
public class ClockEvent : MonoBehaviour
{
    ...
    List<int> deltaHashCods = new List<int>();
    bool isWorking;

    void Update()
    {
        SelfUpdate(Time.deltaTime);
    }

    public void SelfUpdate(float deltaTime)
    {
        DoEventUpdate(deltaTime);
        AddEventUpdate();
        RemoveEventUpdate();
    }
    void DoEventUpdate(float deltaTime)
    {
    ...
    void RemoveEventUpdate()
    {
        if (deltaHashCods.Count == 0)
            return;
    ...
    /// <param name="isUpdateNum">update计数</param>
    /// <returns></returns>
    int AddClockFun(ClockKey clockkey, bool isUpdateNum = false)
    {
        if (isWorking || isUpdateNum)
    ...
    #region 静态实例化方法
    private ClockEvent() { }
    private static ClockEvent clockWorked;
    private static ClockEvent GetInstance()
    {
        if (clockWorked == null)
        {
            GameObject obj = new GameObject("ClockEvent");
            DontDestroyOnLoad(obj);
            clockWorked = obj.AddComponent<ClockEvent>();
        }
        return clockWorked;
    }
    ...
    {
        if (!GetInstance().deltaHashCods.Contains(clockkey))
            GetInstance().deltaHashCods.Add(clockkey);
        return GetInstance().deltaHashCods.Contains(clockkey);
    }

    /// <summary>
    /// update若干次之后再执行相关方法
    /// </summary>
    /// <param name="num">经过的update次数,1表示下一帧执行</param>
    /// <param name="funCall"></param>
    /// <returns></returns>
    public static int AddEventDoAfterUpdateNums(int num, Action funCall)
    {
        ...
        return GetInstance().AddClockFun(new ClockKey(usedfun), true);
    }
    /// <summary>
    /// totalTime时间完成后执行
    /// </summary>
    ...
    public static int AddEventTimeOut(float totalTime, Action funCall)
    {
        ...
        return GetInstance().AddClockFun(new ClockKey(usedfun));
    }
    /// <summary>
    /// 随Update刷新
    /// </summary>
